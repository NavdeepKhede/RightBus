...
class Bus {
  ...
  async getAllBuses() {
    try {
      const query = 'SELECT * FROM buses JOIN routes ON buses.route_id = routes.id;';
  ...
  async getBusesBySrcAndDestination(src, destination, date) {
    try{
      ...
      const query = `
      SELECT b.*,
      b.totalSeats - COALESCE(COUNT(sr.seat_number), 0) AS available_seats, r.src, r.destination
      FROM buses b
